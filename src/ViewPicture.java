...
public class ViewPicture extends JFrame{
	...
	public ViewPicture(Driver d){
		//Initialize the window
		super("FamBook-View Pictures");
		...
		info=null;
		try {
			info = driver.getConnector().getPhotos(driver.getAlbumID(), driver.getMemberID());
		...
		add = new JButton("Add New Picture");
	...
	private void updateTable(String[][] info){
		...
		if(info==null){
			JLabel message=new JLabel("This Member Has No Picture Available.");
	...
	public class AddButtonHandler implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			AddPicture ap = new AddPicture(driver);
			setVisible(false);
			repaint();
		}
	}
	...
	public class OkButtonHandler implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				int status=driver.getConnector().deletePhoto(driver.getAlbumID(), driver.getMemberID(), info[index][0]);
