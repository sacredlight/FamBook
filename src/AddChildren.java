...
public class AddChildren extends JFrame{
	...
	private String[] parentMemberInfo;
	
	public AddChildren(Driver theDriver){
		//Initialize the window
		super("FamBook-Add Child");
		...
		driver = theDriver;
		try {
			parentMemberInfo = driver.getConnector().getMemberInformationById(driver.getAlbumID(), driver.getMemberID());
		} catch (SQLException e1) {
		}
		
		member = new JLabel("Parent Member Info:");
		member.setBounds(60, 10, 150, 50);
		cont.add(member);
		
		memberInformation = new JLabel(parentMemberInfo[1] + " " + parentMemberInfo[2] + ", " + parentMemberInfo[3]);
		...
		enter.addActionListener(new ChildEnterButtonHandler());
		add(enter,0);
		
		//Display the "add" button on window
		add = new JButton("Add Selected as Child");
		add.setBounds(320, 460,175,25);
		add.addActionListener(new ChildAddButtonHandler());
		add(add,0);
	...
	public class ChildEnterButtonHandler implements ActionListener{
		public void actionPerformed(ActionEvent e) {
	...
	public class ChildAddButtonHandler implements ActionListener{
		public void actionPerformed(ActionEvent e) {
		    ...
		    else
			{
				...
				{
					String childID = info[index][5];
					int errorCode=-1;
					try {
						errorCode = driver.getConnector().addParentChildRelationship(driver.getAlbumID(), parentMemberInfo[0], childID);
					} catch (SQLException e1) {
					}	
					
					if (errorCode==-1)
						message.setText("Select Distinct Parent and Child");
					else if (errorCode==1)
						message.setText("Relationship Already Exists");
					else if (errorCode==2)
						message.setText("Child Already Has 2 Parents");
					else						
