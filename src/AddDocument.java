...
public class AddDocument extends JFrame implements DropTargetListener{
	...
	public AddDocument(Driver d) {
		//Initialize the window
		super("FamBook-Add Document");
		...
		enter.addActionListener(new DocumentEnterButtonHandler());
		add(enter,0);
	...
	public class DocumentPathHandler implements ActionListener{
		public void actionPerformed(ActionEvent e) {

		}
	}
	
	public class DocumentEnterButtonHandler implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			if (filePath == null || filePath.compareTo("")==0 || !filePath.toLowerCase().endsWith(".txt"))
			...
			else
			{
				...
				try {
					successful = driver.getConnector().addDocument(driver.getAlbumID(), driver.getMemberID(), filePath, title.getText(), enteredDate);
	    ...
	    for (int i = 0; i < flavors.length; i++) {
	    	...
	    	if (flavors[i].isFlavorJavaFileListType()) {
	    		...
	    		for (int j = 0; j < list.size(); j++) {
	    			...
	    			if (list.get(j).toString().toLowerCase().endsWith(".txt"))
	    			{
	    			...
	    			{
	    				pathDrop.setText("FILE MUST BE TXT");
	    				filePath = "";
	    			}
	    		}
