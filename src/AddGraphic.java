...
public class AddGraphic extends JFrame implements DropTargetListener{
	...
	public AddGraphic(Driver d) {
		//Initialize the window
		super("FamBook-Add Graphic");
		...
		enter.addActionListener(new GraphicEnterButtonHandler());
		add(enter,0);
	...
	public class GraphicPathHandler implements ActionListener{
		public void actionPerformed(ActionEvent e) {

		}
	}
	
	public class GraphicEnterButtonHandler implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			if (filePath == null || filePath.compareTo("")==0 || !filePath.toLowerCase().endsWith(".gif"))
			...
			else
			{
				...
				try {
					successful = driver.getConnector().addGraphic(driver.getAlbumID(), driver.getMemberID(), filePath, title.getText(), enteredDate);
	    ...
	    for (int i = 0; i < flavors.length; i++) {
	    	...
	    	if (flavors[i].isFlavorJavaFileListType()) {
	    		...
	    		for (int j = 0; j < list.size(); j++) {
	    			...
	    			if (list.get(j).toString().toLowerCase().endsWith(".gif"))
	    			{
	    			...
	    			{
	    				pathDrop.setText("FILE MUST BE GIF");
	    				filePath = "";
	    			}
	    		}
