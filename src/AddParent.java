...
public class AddParent extends JFrame{
	...
	private String[] childMemberInfo;
	
	public AddParent(Driver theDriver){
		//Initialize the window
		super("FamBook-Add Parent");
		//setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		
		driver = theDriver;
		try {
			childMemberInfo = driver.getConnector().getMemberInformationById(driver.getAlbumID(), driver.getMemberID());
		...
		member = new JLabel("Child Member Info:");
		member.setBounds(60, 10, 150, 50);
		cont.add(member);
		
		memberInformation = new JLabel(childMemberInfo[1] + " " + childMemberInfo[2] + ", " + childMemberInfo[3]);
		...
		enter.addActionListener(new ParentEnterButtonHandler());
		add(enter,0);
		
		//Display the "add" button on window
		add = new JButton("Add Selected As Parent");
		add.setBounds(320, 460,175,25);
		add.addActionListener(new ParentAddButtonHandler());
		add(add,0);
	...
	public class ParentEnterButtonHandler implements ActionListener{
		public void actionPerformed(ActionEvent e) {
	...
	public class ParentAddButtonHandler implements ActionListener{
		public void actionPerformed(ActionEvent e) {
		    ...
		    else
			{
				...
				{
					String parentID = info[index][5];
					int errorCode=-1;
					try {
						errorCode = driver.getConnector().addParentChildRelationship(driver.getAlbumID(), parentID, childMemberInfo[0]);
					} catch (SQLException e1) {
					}	
