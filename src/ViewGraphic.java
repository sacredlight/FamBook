...
public class ViewGraphic extends JFrame{
	...
	public ViewGraphic(Driver d){
		//Initialize the window
		super("FamBook-View Graphics");
		...
		info=null;
		try {
			info = driver.getConnector().getGraphics(driver.getAlbumID(), driver.getMemberID());
		...
		add = new JButton("Add New Graphic");
	...
	private void updateTable(String[][] info){
		...
		if(info==null){
			JLabel message=new JLabel("This Member Has No Graphic Available.");
			message.setBounds(200,200,260,25);
			add(message,0);
		}else{
	...
	public class AddButtonHandler implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			AddGraphic ap = new AddGraphic(driver);
			setVisible(false);
			repaint();
		}
	}
	...
	public class OkButtonHandler implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				int status=driver.getConnector().deleteGraphic(driver.getAlbumID(), driver.getMemberID(), info[index][0]);
